...
from custom_components.gardena_smart_system_ng.const import DOMAIN, WS_WATCHDOG_TIMEOUT_SECONDS
...
class TestWsUrlAlwaysFresh:
    ...
    async def test_watchdog_disconnect_triggers_fresh_url_fetch(
        ...
        with (
            ...
            with patch(
                "custom_components.gardena_smart_system_ng.base_coordinator.time.monotonic",
...
class TestCustomPollIntervalWithWs:
    ...
    async def test_ws_disconnected_uses_custom_interval(
        ...
        from custom_components.gardena_smart_system_ng.const import OPT_POLL_INTERVAL_MINUTES
...
class TestWsHandshakeKillSwitch:
    ...
    async def test_kill_switch_activation_creates_app_blocked_issue(
        ...
        from custom_components.gardena_smart_system_ng.const import WS_HANDSHAKE_DENIAL_THRESHOLD
...
class TestWsLostLogSeverity:
    ...
    async def test_first_drop_logs_at_debug(
        ...
        with (
            ...
            with caplog.at_level(
                _logging.DEBUG, logger="custom_components.gardena_smart_system_ng.base_coordinator"
            ):
                coordinator._on_ws_error(self._make_handshake_error(410))
    ...
    async def test_persistent_drops_escalate_to_warning(
        ...
        from custom_components.gardena_smart_system_ng.const import WS_REPAIR_ISSUE_THRESHOLD
